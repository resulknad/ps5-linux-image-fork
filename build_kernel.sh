...
Options:
  ...
  --kernel <path>      Use this kernel source directory (implies --local)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --kernel)         KERNEL_SRC_ARG="$2";     shift 2 ;;
...
if [ -n "$KERNEL_SRC_ARG" ]; then
    LOCAL=true
    KERNEL_SRC="$(cd "$KERNEL_SRC_ARG" && pwd)"
fi

if [ "$LOCAL" = true ] && [ -n "$PATCHES_DIR_ARG" ]; then
    echo "Error: --local/--kernel and --patches-dir are mutually exclusive."
    exit 1
fi

...
usage() {
    ...
    echo "  --local      Skip repull/repatch, use existing kernel source"
    echo "  --patches-dir  Use this patches directory instead of cloning"
    echo "  --format     Package format: deb, arch, all (default: auto from distro)"
    echo "  --patches-token  GitHub token for HTTPS patches repo access"
    echo "  --cross-compile  Run kernel builder as linux/arm64 (native on aarch64 hosts)"
    exit 1
}
